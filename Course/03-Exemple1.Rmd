...
```{r}
cut_disease_duration <- quantile(data$Disease_duration)[2:4]
cut_age <- mean(data$Age)

comparaisons <- emmeans(model2, ~ Mutation_bis | Disease_duration, at = list(Disease_duration = cut_disease_duration))
emmeans::contrast(comparaisons, interaction = c("pairwise"))
```
